"""SubagentStart hook handler for GitHub Copilot — skill loading reminder injection.

Reads hook input JSON from stdin. For nWave sub-agents (agent_type starting
with "nw-"), writes a hookSpecificOutput JSON reminder to stdout instructing
the agent to load its relevant skill files from .github/skills/. 
...
Output format (for nw-* agents only):
    {"hookSpecificOutput": {"hookEventName": "SubagentStart",
     ...
     skill files from .github/skills/{agent_type}/ before beginning work.
...
def _build_reminder_message(agent_type: str) -> str:
    ...
    return (
        ...
        f".github/skills/{agent_type}/ before beginning work. "
        "Skills encode your methodology — load the ones applicable to your current "
        "task at the appropriate phase."
    )


def handle_subagent_start_copilot() -> int:
    """Handle subagent-start hook: inject skill loading reminder for nWave agents.

    Reads JSON from stdin (GitHub Copilot SubagentStart hook protocol). If the
    spawning agent_type starts with "nw-", writes a hookSpecificOutput JSON
