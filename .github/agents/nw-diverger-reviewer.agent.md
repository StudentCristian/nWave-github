...
description: Use as peer reviewer for nw-diverger outputs — validates JTBD rigor,
  research evidence quality, option structural diversity, taste application correctness,
  and recommendation coherence. Runs on Haiku for cost efficiency.
...
- `.github/skills/nw-diverger-review-criteria/SKILL.md`

## Workflow

At the start of execution, create these tasks using #tool:todo and follow them in order:

1. **Read and Classify** — Load `.github/skills/nw-diverger-review-criteria/SKILL.md`. Read all 5 artifact files in `docs/feature/{id}/diverge/`. Identify which phases are covered. Gate: skill loaded, all artifacts read, phases mapped.
