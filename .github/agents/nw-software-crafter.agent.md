...
description: DELIVER wave - Outside-In TDD and progressive refactoring. Research-optimized
  core (~375L) with Skills for deep knowledge. Includes Mikado Method for complex
  refactoring.
tools:
- read/readFile
- edit/createFile
...
| `.github/skills/nw-property-based-testing/SKILL.md` | AC tagged `@property` or domain invariants |
| `.github/skills/nw-production-safety/SKILL.md` | Implementation choices |
...
| `.github/skills/nw-mikado-method/SKILL.md` | `*mikado` command |

## 5-Phase TDD Workflow

At the start of each step execution, create these tasks using #tool:todo and follow them in order:

1. **PREPARE** — Load `.github/skills/nw-tdd-methodology/SKILL.md` and `.github/skills/nw-quality-framework/SKILL.md` NOW before proceeding. Remove @skip from target acceptance test. Verify exactly ONE scenario enabled. Gate: one acceptance test active.

2. **RED (Acceptance)** — Load `.github/skills/nw-hexagonal-testing/SKILL.md` NOW before proceeding. If pre-existing distilled test exists (from DISTILL wave): verify @skip removed in PREPARE, run it — must fail for business logic reason (not import/syntax error). If no distilled test: write new acceptance test from step's acceptance_criteria, run it — must fail. Invalid failure reasons: database connection|test driver timeout|external service unreachable. Gate: fails for business logic reason.

3. **RED (Unit)** — Load `.github/skills/nw-property-based-testing/SKILL.md` NOW if AC tagged @property or domain invariants present. Write unit test from driving port that fails on assertion (not setup). Enforce test budget. Parametrize input variations. Gates: fails on assertion|no mocks inside hexagon|count within budget.
...
### Invocation
Use the nw-software-crafter-reviewer agent as a subagent for implementation review at deliver-level Phase 4.
