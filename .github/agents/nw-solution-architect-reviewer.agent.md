...
- `.github/skills/nw-sar-critique-dimensions/SKILL.md`

### On-Demand (load only when triggered)

| Skill | Trigger |
|-------|---------|
| `.github/skills/nw-roadmap-review-checks/SKILL.md` | When roadmap present — 6 mandatory checks |
...
2. **Architecture Review** — Load `.github/skills/nw-sar-critique-dimensions/SKILL.md` NOW before proceeding. Evaluate 5 dimensions: bias detection, ADR quality, completeness, feasibility, priority validation. Score each with specific findings. Gate: all dimensions evaluated.
3. **Roadmap Review** — Load `.github/skills/nw-roadmap-review-checks/SKILL.md` NOW if roadmap is present. Apply 6 mandatory checks: external validity, AC coupling, step decomposition, implementation code, concision, test boundaries. Gate: all checks applied (skip if no roadmap).
