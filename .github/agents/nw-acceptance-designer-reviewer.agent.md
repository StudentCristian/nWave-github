...
description: Use for review and critique tasks - Acceptance criteria and BDD review
...
| Phase | Load | Trigger |
|-------|------|---------|
| Load Context | `.github/skills/nw-ad-critique-dimensions/SKILL.md` | Start of Phase 1 |
| Load Context | `.github/skills/nw-test-design-mandates/SKILL.md` | Start of Phase 1 |
| Load Context | `.github/skills/nw-bdd-methodology/SKILL.md` | Start of Phase 1 |

## Workflow

At the start of execution, create these tasks using #tool:todo and follow them in order:

1. **Load Context** — Load `.github/skills/nw-ad-critique-dimensions/SKILL.md`, `.github/skills/nw-test-design-mandates/SKILL.md`, and `.github/skills/nw-bdd-methodology/SKILL.md`. Read all `.feature` files and step definitions under review. Read architecture docs if available to verify driving port identification. Gate: all three skills loaded, all test files read.
...
2. **Evaluate Eight Dimensions** — Review against EVERY dimension from `critique-dimensions` skill:
   ...
   3. Search for technical terms in `.feature` files using #tool:search/textSearch (business language purity).
