...
Sub-agents cannot load skills directly or use `/nw:*` commands. You MUST:
- Read the relevant command file and embed instructions in the #tool:agent prompt
- Remind the crafter to load its skills as needed for the task (skill files are at `.github/skills/nw-{agent-name}/`)
...
3. **Extract step context from roadmap.json ONLY for #tool:agent prompt.** Grep roadmap for step_id ~50 lines context, extract (description|acceptance_criteria|files_to_modify), pass in DES template.

**DES monitoring is non-negotiable.** Circumventing DES — faking step IDs, omitting markers, or writing log entries manually — is a **violation that invalidates the delivery**. DES detects unmonitored steps and flags them; finalize **blocks** until every flagged step is re-executed through a properly instrumented #tool:agent. There is no workaround: unverified steps cannot pass integrity verification, and the delivery cannot be finalized. Without DES monitoring, nWave cannot **verify** TDD phase compliance. For non-deliver tasks (docs, research, one-off edits): `<!-- DES-ENFORCEMENT : exempt -->`.
...
| Setting | Effect |
|---------|--------|
| `agent_model` | Pass as `model` parameter to all #tool:agent invocations for crafter agents. If `"inherit"`, omit `model` parameter (agent inherits from session). |
| `reviewer_model` | Pass as `model` parameter to reviewer invocations. If `"skip"`, skip Phase 4 entirely. |
...
| `mutation_enabled` | If `false`, skip Phase 5 regardless of project strategy. |

**Invocation example (conceptual):**
```
# Use #tool:agent to dispatch subagents with the selected model when required
# (example conceptual snippet, not executed literally here)
#tool:agent subagent={agent} model={rigor_agent_model} prompt=... description={phase description}
```
...
INPUT: "{feature-description}"
  ...
  1. Parse input|derive feature-id (kebab-case)|create docs/feature/{feature-id}/deliver/
     a. Create execution-log.json if missing via CLI:
        PYTHONPATH=$HOME/.github/lib/python $(command -v python3 || command -v python) -m des.cli.init_log --project-dir docs/feature/{feature-id}/deliver --feature-id {feature-id}
  ...
  1.6. Detect mutation testing strategy
     a. Same .github/copilot-instructions.md|search "## Mutation Testing Strategy"
     ...
     Note: Strategy locks at deliver start. .github/copilot-instructions.md edits during delivery take effect next run.
  |
  2. Phase 1 — Roadmap Creation + Review
     ...
     b. @nw-solution-architect creates roadmap.json (read .github/skills/nw-roadmap/SKILL.md)
     ...
     d. @nw-software-crafter-reviewer reviews (read .github/skills/nw-review/SKILL.md)
     ...
     **INVARIANT: 1 Agent invocation = 1 roadmap step. No exceptions.**
     ...
     c. **Dispatch loop** — for EACH pending step:
        ...
        c2. Dispatch ONE agent with ONE DES-STEP-ID using DES Prompt Template from execute.md
            ...
            OUTCOME_RECORDING: agents use DES CLI (PYTHONPATH=$HOME/.github/lib/python $(command -v python3 || command -v python) -m des.cli.log_phase)|CLI bypass → SubagentStop hook corrects timestamps
     ...
     e. Timeout recovery: GREEN completed → resume (~5 turns)|GREEN partial → resume|Otherwise → restart
  |
  ...
  5. Phase 4 — Adversarial Review [SKIP if rigor.review_enabled = false]
     ...
     b. /nw-review @nw-software-crafter-reviewer implementation "{execution-log-path}"
        Use model=rigor.reviewer_model for reviewer invocation
  ...
  6. Phase 5 — Mutation Testing [SKIP if rigor.mutation_enabled = false]
     If rigor.mutation_enabled = false → SKIP regardless of project strategy
     Otherwise, apply project strategy:
     per-feature → gate ≥80% kill rate (read .github/skills/nw-mutation-test/SKILL.md)
  ...
  7. Phase 6 — Deliver Integrity Verification
     a. PYTHONPATH=$HOME/.github/lib/python $(command -v python3 || command -v python) -m des.cli.verify_deliver_integrity docs/feature/{feature-id}/deliver/
     b. Exit 0 → proceed|Exit 1 → STOP, read output
     c. No entries = not executed through DES|Partial = incomplete TDD
     d. Violations → re-execute via #tool:agent with DES markers|Only proceed after pass
  |
  8. Phase 7 — Finalize
     a. @nw-platform-architect archives to docs/evolution/ (read .github/skills/nw-finalize/SKILL.md)
...
2. Extract instructions and embed them in the #tool:agent prompt
3. Include task boundary instructions to prevent workflow continuation
4. Verify output artifacts exist after each invocation completes
